/* ═══════════════════════════════════════════════════════
   Albexia — albexia.js  (Firebase Edition — patched)
...
(async function () {
  ...
  // FIX BUG 3 : Attendre le DOM avant de détecter la page
  ...
  async function initProfile(user, profile) {
    ...
    /* ── FIX BUG 2 : Chargement initial du dashboard avec gestion d'erreur ── */
    try {
      ...
      /* FIX : mettre aussi à jour les compteurs dans les onglets si déjà visibles */
      ...
      /* Afficher 0 plutôt que de laisser vide — mieux pour l'UX */
  ...
  /* FIX BUG 1 : Pour l'index, exposer user AVANT d'émettre l'événement
     afin que app.js puisse détecter _fbUser dans loadAllData() */
  window._sbUser = user;
  window._fbUser = user;

  if (IS_INDEX) initIndexFavorites(user);

  /* Émettre un événement pour app.js — après avoir défini _fbUser et _sbUser */
