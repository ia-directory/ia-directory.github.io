...
// IDs "de base" (sans suffixe -en/-es) des outils actuellement affichés
// dans le panneau spotlight, si présent. Permet de reconstruire le panneau
// dans la nouvelle langue quand l'utilisateur change de langue (FR/EN/ES)
// sans perdre le contexte "vient d'un article".
let _spotlightBaseIds = null;

// Retire un éventuel suffixe -en / -es pour obtenir l'id "de base" (FR).
function toBaseId(id) {
  return String(id).replace(/-(en|es)$/, '');
}

// Reconstruit l'id attendu pour une langue donnée à partir de l'id de base.
function idForLangue(baseId, code) {
  return code === 'fr' ? baseId : `${baseId}-${code}`;
}
...
function changerLangue(code) {
  ...
  // reconstruit dans la nouvelle langue à partir des ids de base mémorisés,
  // au lieu de le laisser dans l'ancienne langue ou de le perdre.
  if (_spotlightBaseIds && _spotlightBaseIds.length) {
    const found = _spotlightBaseIds
      .map(baseId => state.tools.find(t => String(t.id) === idForLangue(baseId, code)))
      .filter(Boolean);
    if (found.length) {
...
function checkToolsParam() {
  ...
  // Mémorise les ids de base (sans suffixe -en/-es) pour pouvoir reconstruire
  // le panneau dans la bonne langue si l'utilisateur change de langue ensuite.
  _spotlightBaseIds = ids.map(toBaseId);
